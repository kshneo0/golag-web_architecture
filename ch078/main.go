...
func main() {
	...
	if errors.Is(err, os.ErrPermission) && errors.As(err, &perr) {
		...
		// return fmt.Errorf("here is the original error %w and the path %s", err, perr.Path) RETURNS A STRING
		err = fmt.Errorf("here is info original error %w ans the path %s", err, perr.Path)
		log.Println(err)
	} else if err != nil {
